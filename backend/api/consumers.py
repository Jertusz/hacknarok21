...
from .models import CustomQuestionLog
...
def get_random_question(session_id):
    ...
    new_activity_question = ActivityLog(
        session_id=session, no_answer=session.users, question=random_question
    )
...
class SessionConsumer(AsyncWebsocketConsumer):
    ...
    async def connect(self):
        ...
        await database_sync_to_async(create_or_update_session)(
            self.session_id, self.user_name, self.is_admin
        )
    ...
    async def receive(self, text_data):
        ...
        if event_type == "activity_question_answered":
            ...
            custom_answer_in_db = CustomQuestionLog(
            ...
            if self.is_admin:
                question_in_db = CustomQuestion(
                    session_id=self.session_id, content=question
                )
    ...
    async def generic(self, event):
        ...
        text = event["text"]
        await database_sync_to_async(generic_log)(
            self.user_name, self.session_id, action, text
        )
        await self.send(
            text_data=json.dumps(
                {
